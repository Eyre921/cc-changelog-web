...
一个交互式的 Claude Code 功能速览网站，梳理了从 **v0.2.21 到 v2.1.74** 共 **220+ 个版本**、**1000+ 项更新**的完整功能演进。每项功能都配有真实使用场景，用中国开发者熟悉的语言讲述，看完就能上手。
...
- **丰富微交互** — 滚动进度条、卡片入场交错、hover 光波、统计数字滚动计数、时间线脉冲、hero 浮动粒子
...
- **新增 2 个能力亮点补强** —— `/context` 命令从"看数字"升级为"开处方"，`modelOverrides` 让模型选择器可映射到任意提供商模型 ID（如 Bedrock 推理配置 ARN），`autoMemoryDirectory` 让 auto-memory 文件存放路径可自定义
- **重点修复集中在稳定性** —— 流式 API 内存泄漏（RSS 无限增长）、权限提示 100% CPU 死循环、skill 文件批量变更死锁、MCP OAuth 挂起、RTL 文字渲染、Windows LSP URI 等，重度使用场景毛刺再磨一轮
- **暂时不需要新增模块** —— 现有 13 大板块已经能完整承接 v2.1.73 — v2.1.74 的所有更新
- **文案继续往「更生动、更场景化、更像中国开发者聊天」靠拢** —— 新卡片采用"上下文吃紧时自动报警""模型映射自由度拉满""稳定性大扫除"等社区化表达
- **动效升级** —— 新增滚动进度条、卡片入场交错动画、hover 光波扫过、统计数字滚动计数、时间线脉冲、hero 浮动粒子等微交互，让页面更生动好看
