...
- **重点补强 6 个现有板块** —— 终端体验（bash 自动放行扩容）、多 Agent（team agents 模型继承）、性能（prompt cache 12 倍成本降低 + 包体积 -510KB）、VS Code（effort 指示器 + URI handler）、MCP 生态（marketplace 免 .git 后缀）、安全沙箱（CLAUDE.md 注释隐藏）
- **文案继续往「更生动、更场景化、更像中国开发者聊天」靠拢** —— 少写发布说明腔，多写「SSH 党狂喜」「想到就开工」「一眼看懂档位」「查端口免签」这类社区常用表达
