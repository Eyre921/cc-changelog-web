...
一个交互式的 Claude Code 功能速览网站，梳理了从 **v0.2.21 到 v2.1.111** 共 **258+ 个版本**、**1300+ 项更新**的完整功能演进。每项功能都配有真实使用场景，用中国开发者熟悉的语言讲述，看完就能上手。
...
- **14 大核心功能板块 + 最近更新专栏** — AI 模型、多 Agent 协作、会话管理、开发工作流、MCP 生态、终端体验、安全沙箱、SDK 集成、企业工程化与可观测、性能优化、跨平台、VS Code 扩展等
...
- **Opus 4.7 xhigh + Auto Mode 上线** —— v2.1.111 提供 xhigh 推理档位（介于 high 与 max 之间），复杂任务更稳；Max 订阅还能启用 Auto Mode，根据任务自动平衡速度与深度。
- **`/ultrareview` 云端并行审查** —— 新增综合代码审查能力，可对当前分支或指定 PR 做多 Agent 并行评审，适合合并前做“最后一道质量闸门”。
- **`/less-permission-prompts` 减少权限打断** —— 自动扫描会话并生成 allowlist 建议，把高频只读调用从重复弹窗里解放出来，同时保持策略边界。
- **`/effort` 交互滑块体验升级** —— 无参调用可打开滑块，方向键切档、回车确认；新加入 xhigh 档位后，推理强度调节更细腻。
- **企业工程化板块独立成章** —— 把 Perforce 模式、Monitor 流式观测、OTel 链路追踪、权限治理自动化集中呈现，内容更场景化、叙事更完整。
- **全屏与焦点阅读体验增强** —— `/tui fullscreen` 与 `/focus` 分离，配合自动滚动与编辑器上下文回填，长会话阅读和排障更沉浸。
...
| 14 | 版本时间线 | v0.2.21 ~ v2.1.111 完整历程 |
